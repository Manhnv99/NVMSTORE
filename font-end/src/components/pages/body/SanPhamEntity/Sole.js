...
const Sole=()=>{
    ...
    const handleAdd= async ()=>{
        if(addOrUpdate){
            ...
            try {
                const response = await soleAPI.addSole(categoryRequest)
            ...
            try{
                const response=await soleAPI.updateSole(idUpdate,categoryRequest);
    ...
    const handleEdit= async (id)=>{
        ...
        try {
            const res= await soleAPI.getById(id)
        ...
        <>
                ...
                <div className="base-body">
                    <div className="base-body-title">
                        <div className={"base-body-title-left"}>
                            ...
                            <span>Danh sách đế giày</span>
                        </div>
                        <div className={"base-body-title-right"}>
                            <button onClick={handleShow} className={"btn btn-primary"}>
                                <i className="fa-solid fa-plus"></i>
                                <span style={{marginLeft: "5px"}}>Thêm đế giày</span>
                            </button>
                        </div>
                    </div>
                        ...
                        <Card.Body style={{padding:"0"}}>
                            <Table style={{marginBottom:"0"}}>
                                ...
                                <tr style={{fontSize:"13px",fontWeight:"600"}}>
                                    ...
                                    <th style={{width:"30%",textAlign:"center"}}>Tên Đế Giày</th>
            ...
            <Modal show={show} onHide={handleClose}>
                <Modal.Header closeButton>
                    <Modal.Title>
                        {addOrUpdate===true ? 'Thêm đế giày' : "Cập nhật đế giày"}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Col sm={12}>
                        <Form.Group>
                            <Form.Label><span style={{color: "red"}}>*</span> Tên đế giày</Form.Label>
                            <Form.Control value={name} onChange={onChangeName} required isInvalid={touchName && name==='' || error !== undefined} isValid={name!==''} type="text" placeholder="Điền tên đế giày!"/>
                            <Form.Control.Feedback type={"invalid"}>
                                {error !== undefined ? error : "Tên Đế Giày Không Được Để Trống!"}
